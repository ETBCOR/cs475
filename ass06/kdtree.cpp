...
void search(Matrix *t, Matrix x, int *bestrow, float *bestdist, int lower, int upper, int &n, int c) {
	int size = upper-lower+1;
	int m = size / 2.0f + lower;
	c = (c >= 3) ? 1 : c + 1; // increment c
	...
	// else if parent node case
		...
		if (x.get(0, c-1) < t->get(m, c)) {
			...
			search (t, x, bestrow, bestdist, lower, m - 1, n, c);
			...
			search (t, x, bestrow, bestdist, m + 1, upper, n, c);
		} else {
			// do right then left
